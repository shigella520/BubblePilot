...
const filtered = computed(() =>
  props.blocks
    .filter((block) =>
      `${block.name} ${block.description} ${block.category}`
        .toLowerCase()
...
<template>
  ...
  >
    ...
    <button
      ...
      :key="`${block.type}@${(block as any).version ?? 1}`"
      class="workflow-node-result"
      type="button"
    ...
    <p v-if="!filtered.length" class="workflow-node-creator-empty">
      没有匹配的动作
    </p>
  </aside>
</template>
