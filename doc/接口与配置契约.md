...
统一消息信封当前为 `schemaVersion=2`，新增必填 `message.linkPreview`。状态为 `not-requested|pending|available|unavailable|failed|redacted`，最多保存 4 条规范化预览；单条只包含来源、HTTP(S) URL、有限标题/摘要/站点名和图片/图标存在标志。原始 BlueBubbles `payloadData`、网页 HTML 和媒体二进制不进入该合同。
