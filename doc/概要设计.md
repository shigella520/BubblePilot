...
AI 节点的 `webSearch` 策略为 `disabled`、`auto` 或 `required`，默认关闭。`webSearchSources` 为 `full`、`compact` 或 `hidden`，分别完整展示、末尾精简展示或仅在执行详情保留搜索来源。启用时，实例环境变量 `ENABLE_WEB_SEARCH` 仍是总开关。V1.1 优先使用探测成功的 Provider Responses 托管搜索；其他 Provider 必须通过独立的 Function Calling 探测，由进程内 AgentRunner 调用同 Stack 的 SearXNG 并回填结果。工具调用有轮次、次数、超时和输出长度上限，不执行任意脚本。搜索适配器把 HTTP 200 的零结果作为可恢复的查询结果，并把部分引擎错误作为诊断而非后端故障；AgentRunner 只把至少一条可用结果计为 `required` 成功，且允许在已有证据时忽略后续收窄查询的故障。`site:` 查询由适配器在同一超时预算内执行一次保域名语义的放宽查询，避免把不同搜索引擎对高级操作符的支持差异泄漏给工作流提示词。为支持问题排查，工具轨迹保存实际搜索参数、查询策略、规范化后的有限结果和引擎错误，但不保存原始聊天正文、完整 Provider Prompt、Secret 或搜索后端未裁剪的原始响应。
