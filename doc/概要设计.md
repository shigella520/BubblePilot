...
AI Provider Router 负责锁定路由快照、按固定顺序选择启用候选、执行有界 Fallback、汇总失败分类和维护运行时健康状态。它必须把管理员配置的顺序与自动降级产生的有效顺序分开保存，不能因临时故障永久改写配置。

每次 Provider 调用均产生独立尝试记录，至少包含路由版本、Provider、模型、候选序号、节点重试轮次、耗时、结果和脱敏错误码。路由器不能读取或改变消息权限，也不能绕过 AI 输出安全检查。
