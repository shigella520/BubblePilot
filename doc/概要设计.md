...
AI 节点的 `webSearch` 策略为 `disabled`、`auto` 或 `required`，默认关闭。启用时，实例环境变量 `ENABLE_WEB_SEARCH` 仍是总开关；V1 只使用 Provider Responses 接口托管的 Web Search。Provider 的能力声明和最近一次能力探测分别保存，路由只选择能力声明为支持且探测成功的候选。V1 不执行任意本地脚本、不使用 Legacy 提示词模拟工具调用。
