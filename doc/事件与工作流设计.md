...
下游 AI 请求按稳定前缀组织：固定 System 和任务说明在前，摘要单独成块，每条原始历史分别映射为稳定的 `user` 或 `assistant` 消息块，当前消息复用相同格式并在末尾追加当前轮标记。不得把全部历史拼成每轮重写的单个字符串，也不得按窗口位置重新编号。摘要未变化时，新请求只在已有历史后追加；摘要、成员映射或工作流提示词变化时允许重新建立 Provider Prompt Cache 前缀。摘要不携带成员姓名或昵称，成员识别仍只来自本轮实际原始消息。
