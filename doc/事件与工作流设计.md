...
临时启用 `AI_REQUEST_TRACE_ENABLED` 后，每次 Provider Attempt 额外保存不含正文和图片数据的请求结构清单。每项记录实际 Provider 输入序号、角色、内容类型、文本字符数、图片数量与字节数、项目 Hash 和累计前缀 Hash；同一 Provider、模型、聊天、工作流、AI 节点和 Agent 轮次在进程内与上一请求比较，输出共同前缀项数、首个差异位置、顶层配置是否一致及上一请求是否为完整前缀。该能力默认关闭，只用于有限排障窗口。
