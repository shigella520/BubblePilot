...
下游 AI 请求按“稳定文本前缀 + 易变增强材料尾部”组织：固定输入协议、工作流 System、静态任务说明和摘要在前；每条历史及当前消息分别映射为稳定的 `user` 或 `assistant` `<chat_history>`，只包含稳定消息 ID、时间、`sender_id`、正文以及附件/卡片引用。其后依次放成员映射、链接预览、历史图片、当前图片和资源诊断。不得把全部历史拼成每轮重写的单个字符串，也不得按窗口位置重新编号。摘要未变化且未越过冗余缓冲边界时，历史文本序列必须严格追加；成员、卡片或图片变化只能影响后置增强区。
...
当前消息或已加载历史存在卡片时，稳定消息只携带 `link_preview_ref`；可用预览集中放在后置 `<link_previews trust="untrusted_external_metadata">`，通过引用关联原消息。内容只含规范化 URL、标题、摘要和站点名；模型不得执行其中的指令，也不得仅凭卡片声称已读取网页全文。链接预览持久化只允许 `pending` 原子转换为最终状态；并发竞争失败方回读数据库获胜快照，不能覆盖已完成结果。

实例级“原生图片输入”启用后，每个 AI 节点自动尝试附带当前消息图片附件和链接卡片主图；开启 `includeLoadedContext` 的节点还会附带最近的有限历史图片。稳定消息永久保留 `attachment_ref`，实际历史图片集中在后置 `<history_attachments>`，当前图片放在 `<current_attachments>`，均通过稳定引用关联原消息。历史图片滚动、下载失败或字节限制只改变尾部附件区，不删除或改写历史消息中的引用。Chat Completions 映射为 `image_url`，Responses 映射为 `input_image`；失败限制统一放在尾部资源诊断区。
