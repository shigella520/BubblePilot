...
实例级“原生图片输入”启用后，每个 AI 节点自动尝试附带当前消息图片附件和链接卡片主图；开启 `includeLoadedContext` 的节点还会按全局限制附带有限历史图片。历史图片与所属 `<chat_history>` 绑定；超过数量/字节限制、下载失败或失效时，使用稳定摘要或固定占位文本，不再追加独立 `<history_attachments>` 区。当前图片放在 `<current_attachments>` 尾部。Chat Completions 映射为 `image_url`，Responses 映射为 `input_image`；失败限制统一放在尾部资源诊断区。
