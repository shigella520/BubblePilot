...
当前 `WorkflowExecutionDispatcher` 是进程内实现，并由并发数、等待队列和等待时间限制容量。同一 `Workflow ID + 消息 Provider + Chat ID` 的执行在门控内严格串行，确保前一轮完成、回复落库和上下文状态稳定后才开始下一轮；不同工作流或聊天仍可并行。被同键占用的队列项不会阻塞其他可运行键取得全局执行槽。人工重试遵循相同隔离键。未来独立 Worker 必须实现同一语义和持久化原子 Claim，不能只迁移内存队列。
