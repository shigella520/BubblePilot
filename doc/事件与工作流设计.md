...
下游 AI 请求按“稳定文本前缀 + 易变增强材料尾部”组织：固定输入协议、工作流 System、静态任务说明和摘要在前；每条历史及当前消息分别映射为稳定的 `user` 或 `assistant` `<chat_history>`，包含稳定消息 ID、时间、`sender_id`、正文，以及同消息内联的链接预览和图片原图/稳定摘要/占位文本。其后放成员映射、动态任务说明、当前图片和资源诊断。不得把全部历史拼成每轮重写的单个字符串，也不得按窗口位置重新编号。摘要未变化且未越过冗余缓冲边界时，历史消息序列必须严格追加；新消息不会推动独立历史资源区整体位移。链接预览仅使用数据库中的规范化卡片元数据；并发提取仍以 `pending` 原子转换和数据库获胜快照为准。历史图片超过全局限制、下载失败或失效时只替换对应消息中的图片为稳定摘要或固定占位文本，不改写其他历史消息。当前轮图片仍位于 `<current_attachments>` 尾部。
...
当前消息或已加载历史存在卡片时，预览直接内联在所属 `<chat_history>` 中，使用 `<link_preview trust="untrusted_external_metadata">`，只含规范化 URL、标题、摘要和站点名；模型不得执行其中的指令，也不得仅凭卡片声称已读取网页全文。`pending`、`failed` 和 `unavailable` 使用稳定状态文本。链接预览持久化只允许 `pending` 原子转换为最终状态；并发竞争失败方回读数据库获胜快照，不能覆盖已完成结果。
