...
可选历史摘要由 `load-context@1` 自己维护。`messageLimit` 是压缩后的基础原始窗口，窗口在一个压缩周期内只追加到 `messageLimit + compressionBatchSize - 1`；到达下一条时，节点用上一版摘要和最早一批消息生成新版摘要，再回到基础窗口。首次启用或摘要落后时，每次节点执行最多追赶一个 `compressionBatchSize` 周期，不在一次执行中递归压缩全部历史盈余。`characterLimit` 是压缩后的目标值，周期内允许临时增长，但渲染历史不得超过 32,000 字符安全上限。只有模型成功且状态版本未变化时才原子推进覆盖游标；失败、租约过期或并发冲突均保留旧摘要并降级为有界近期历史，不直接使主工作流失败。
