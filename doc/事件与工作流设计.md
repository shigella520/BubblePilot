...
| `load-context@1`    | 读取当前聊天最近消息、已提交聊天级摘要和成员身份 | 1-50 条、100-20,000 字符；严格按消息数和字符上限保留最新完整消息 |
...
聊天摘要由消息归档后的进程内后台任务维护，`load-context@1` 只读取已提交版本。读取历史时先按 `messageLimit` 获取候选消息，再从最新消息向更早消息回溯，按完整消息单元裁剪到 `characterLimit`，不截断单条消息；字符上限优先保留最新的最长连续消息后缀。摘要任务使用固定消息游标、策略版本和基础摘要版本，失败或版本冲突不推进游标。32,000 字符仅作为极端安全兜底，普通字符裁剪不标记工作流失败。
