...
实例级“原生图片输入”启用后，每个 AI 节点自动尝试附带当前消息图片附件和链接卡片主图；开启 `includeLoadedContext` 的节点还会附带最近的有限历史图片。选择顺序是当前附件、当前卡片主图、最近历史图片；同一条历史消息先选择链接卡片主图，再选择普通附件。数量、字节、下载超时、可信链接图片精确域名和图片细节由 AI 页面统一配置。图片以带标签的不可信用户材料独立附加，Chat Completions 映射为 `image_url`，Responses 映射为 `input_image`。部分图片加载失败时，成功图片仍可使用，但模型会被明确要求区分失败图片，不得声称已经看过。
