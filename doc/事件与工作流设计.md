...
| `set-variable@1` | 写入有限 Context 变量 | 变量名和模板键必须通过校验 |
...
单个节点最多 4 个模型轮次和 3 次真实工具调用。搜索超时、结果数量、字段长度和正文长度都有硬上限；外部网页一律是不可信材料，不能覆盖系统指令。达到工具上限时，AgentRunner 会把未执行原因返回模型，并保留一次使用现有结果完成回答的机会。

`webSearchSources=full|compact|hidden` 只控制聊天回复中的来源展示，不删除管理端工具轨迹。工具轨迹保存实际查询、语言与引擎参数、规范化后的有限结果、结果数量和不可用引擎，不保存原始聊天正文或 SearXNG 未裁剪响应。
