...
可选历史摘要由 `load-context@1` 自己维护。未摘要消息达到“近期保留条数＋压缩批量”时，节点用上一版摘要和最早一批消息生成新版摘要；只有模型成功且状态版本未变化时才原子推进覆盖游标。下游上下文按 `<history_summary>`、近期 `<chat_history>`、当前输入排列。摘要生成失败、租约过期或并发版本冲突均不推进游标，并降级为旧摘要和字符预算内的最新原始消息，不直接使主工作流失败。摘要不携带成员姓名或昵称，成员识别仍只来自本轮实际原始消息。
...
对“加载上下文 → AI → 回复”的流程，历史只加载一次，通常只保留一个生成最终回复的 AI 节点，并把输出上限设为场景所需的最小值。Provider Prompt 缓存命中只能通过 `cachedPromptTokens`、`cacheWritePromptTokens` 和 `cacheMissPromptTokens` 观测；历史摘要另有进程内读取缓存，但 PostgreSQL 始终是权威来源，缓存丢失不影响正确性。
