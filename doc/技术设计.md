...
入站归档采用“基础消息先落库、有限预览后补充、再调度工作流”。链接预览优先来自 BlueBubbles `payloadData`，缺失时可由受 SSRF 防护的 Open Graph 客户端访问公网 URL；任何预览失败都只更新消息诊断，不改变入站或工作流结果。原始归档、网页 HTML 与图片不落库，正文保留任务会同时清除规范化预览内容。
