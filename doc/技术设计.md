...
- 会话历史摘要以 PostgreSQL 中聊天级稳定消息索引、当前摘要、覆盖游标和状态版本为权威状态；消息归档后按全局摘要策略异步压缩。每次执行以触发消息索引作为历史独占上界，禁止串行排队期间的新消息回流到较早事件的上下文。进程内 LRU 只缓存可重建的摘要副本。状态按实例、聊天和摘要策略版本隔离，不按工作流或上下文节点隐式复制。
