...
首次昵称可补充准确匹配历史的空昵称，此后改名不改历史快照。后台归属任务采用租约和分批游标，清理后的正文不恢复。身份变化影响已有摘要或检索片段时提升聊天身份修订号，撤销旧派生数据和运行中任务；新结果提交时复核修订号，避免迟到结果覆盖。未知来源的诊断原因不参与证据身份哈希。

迁移隔离旧格式摘要和向量片段；管理员手动启动重建，迁移与昵称保存不调用模型。共享聊天原文仍可见，只有当前 workflowId 的消息作为 assistant，其他角色及未知本账号消息作为带作者信息的 user 参考资料。角色隔离是来源标识与提示协议，不是跨角色消息访问权限，也不承诺消除模型混淆。接口及维护入口见[接口与配置契约](接口与配置契约.md)，重建和回滚见[部署与运维](部署与运维.md)。
...
AI 路由层为普通回复、聊天摘要和图片摘要统一保存 `ai_route_traces`。每条记录固定执行时的路由名称与版本、请求能力要求、候选顺序、能力与健康过滤原因、原生图片或纯文本降级阶段以及最终状态；Provider Attempt 通过追踪 ID 和阶段与之关联。追踪只保存配置诊断和状态，不保存 Secret、图片内容、聊天正文、完整 Prompt 或 Provider 原始响应。旧记录不使用当前路由配置反推历史决策。
...
2. 稳定前缀：固定协议、系统提示、静态任务、摘要和既有历史保持确定顺序；链接预览、历史图片或其摘要固定在所属历史消息中，当前轮动态输入继续后置。
...
路由将图片、联网搜索和健康状态能力条件取交集。普通对话的视觉候选全部失败后可移除图片并走文本候选，且明确限制模型不得伪称查看过图片；`image-summary` 用途禁止该降级。`AiImageInput` 只保存来源类型、状态、MIME、字节、耗时、主机名、来源哈希和错误码。`AiProviderAttempt.purpose` 区分对话回复、历史压缩和图片摘要；图片摘要计入 Provider 总用量，但无工作流执行 ID，不进入执行列表的回复缓存命中率。
...
- 会话历史摘要以 PostgreSQL 中聊天级稳定消息索引、当前摘要、覆盖游标和状态版本为权威状态；消息归档后按全局摘要配置异步压缩。每次执行以触发消息索引作为历史独占上界，禁止串行排队期间的新消息回流到较早事件的上下文。进程内 LRU 只缓存可重建的摘要副本。运行时状态按实例和聊天隔离，不按工作流或上下文节点隐式复制；配置保存只影响后续压缩，不建立新摘要代次或重建历史。
...
新增默认数据库镜像及迁移运维步骤见[部署与运维](部署与运维.md)。普通迁移不依赖 vector 类型，专用准备命令独立建立向量扩展和表。后台失败不阻断归档；普通正文和摘要仍由原有模块负责。
...
AgentRunner 使用每次运行独立的预算对象与配置快照，将联网及归档工具的调用、返回内容和执行时间统一管理；默认 10 次、24,000 字符、60 秒。Provider 内部重试与工作流重试属于各自原有层次；每个重新开始的 Agent 运行有独立快照，不将一个节点的预算冒充整个工作流预算。没有应用侧工具时直接调用模型，仍记录零工具使用的快照。
