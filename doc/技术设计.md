...
需要多账号池粘性路由的兼容 Provider 可以显式启用 `session-id-header` 会话亲和。系统按 Provider、BlueBubbles 聊天、工作流和 AI 节点生成稳定的不可逆 `Session-Id`；它不会包含原始聊天标识，也不随执行、Agent 轮次、Retry 或工具调用变化。单次请求的 `X-Client-Request-Id` 继续保持唯一，用于审计和排障。其他 Provider 默认不发送非标准会话头。
