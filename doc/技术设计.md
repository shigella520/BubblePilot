...
入站归档采用“基础消息先落库、有限预览后补充、再调度工作流”。链接预览优先来自 BlueBubbles `payloadData`，缺失时可由受 SSRF 防护的 Open Graph 客户端访问公网 URL；任何预览失败都只更新消息诊断，不改变入站或工作流结果。原始卡片载荷、网页 HTML 与图片二进制不落库，正文保留任务会同时清除规范化预览内容和卡片主图 URL。
...
需要多账号池粘性路由的兼容 Provider 可以显式启用 `session-id-header` 会话亲和。系统按 Provider、BlueBubbles 聊天、工作流和 AI 节点生成稳定的不可逆 `Session-Id`；Responses 同时使用该值作为 `prompt_cache_key`，让账号池亲和与上游长前缀缓存采用同一个逻辑会话键。该值不会包含原始聊天标识，也不随执行、Agent 轮次、Retry 或工具调用变化。单次请求的 `X-Client-Request-Id` 继续保持唯一，用于审计和排障。其他 Provider 默认不发送会话头或缓存键。
