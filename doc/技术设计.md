...
原生图片输入是 `modules/ai` 内的进程内适配能力，不增加服务边界。启用时，节点从 BlueBubbles 临时读取有限附件，并通过与 Open Graph 相同的 SSRF 防护读取公开卡片主图；魔数、单图大小、总大小和请求超时均在送入 Provider 前校验。Chat Completions 与 Responses 分别使用其原生图片内容块，Agent 多轮和同节点 Provider Fallback 复用已准备的 Data URL，不重复下载。
