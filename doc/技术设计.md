...
`memory_settings` 保存默认关闭的功能设置；`memory_generations` 固定模型、维度、问题前缀、配置和加密凭据。Ollama 读取 `/api/show` 的模型元数据及 Modelfile，计算稳定身份指纹；这是变化检测，不声明所有服务都提供可验证权重 digest。兼容服务依赖管理员模型版本标识，不保证检测供应商同名替换。改变编码配置产生新代次，显式补建和覆盖检查后切换。
...
消息清理与片段文本、关键词失效同事务执行，向量随后由 Worker 回收。查询必须过滤有效来源；执行内证据在再次发送 Provider 前重查源 Hash。历史证据不进入普通诊断或原始请求缓存。旧代次、索引任务和来源引用不能绕过内容保留或聊天授权。
