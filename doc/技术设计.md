...
原生图片输入是 `modules/ai` 内的进程内适配能力，不增加服务边界。启用时，节点从 BlueBubbles 临时读取有限附件，并通过与 Open Graph 相同的 SSRF 防护读取公开卡片主图；异步图片摘要复用相同的下载、可信主机、魔数、大小和超时合同。Chat Completions 与 Responses 分别使用其原生图片内容块，Agent 多轮和同节点 Provider Fallback 复用已准备的 Data URL，不重复下载。原图与 Base64 始终不落库；`MessageImageSummary` 只保存稳定引用、来源键、内容 Hash、有限摘要、状态、模型快照、耗时、错误码和租约信息。
