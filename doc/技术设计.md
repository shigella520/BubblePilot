...
- 会话历史摘要以 PostgreSQL 中的稳定消息索引、当前摘要、覆盖游标和状态版本为权威状态；每次执行以触发消息索引作为历史独占上界，禁止串行排队期间的新消息回流到较早事件的上下文。进程内 LRU 只缓存可重建的摘要副本。状态按实例、Provider、聊天、工作流、上下文节点和摘要策略哈希隔离，禁止不同粒度隐式共享。
...
- 管理 API、Webhook、工作流并发、等待队列和等待时间均有硬上限。工作流执行除全局容量外，还按 `Workflow ID + 消息 Provider + Chat ID` 串行，避免同一 AI 会话缓存键的请求并发交错；不同隔离键仍可并行，队列调度会跳过暂时被占用的键。
