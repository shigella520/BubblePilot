...
REQUIRED_TABLES=$(docker compose exec -T postgres sh -c \
  'psql -v ON_ERROR_STOP=1 -At -U "$POSTGRES_USER" -d "$1" -c "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = '\''public'\'' AND table_name IN ('\''messages'\'', '\''chat_participant_identities'\'', '\''workflow_executions'\'', '\''ai_providers'\'', '\''admin_sessions'\'', '\''data_export_jobs'\'');"' \
  -- "$RESTORE_DATABASE")

if [ "$REQUIRED_TABLES" -ne 6 ]; then
