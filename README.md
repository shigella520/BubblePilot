...
| Top 10 功能亮点 | 你可以用它做什么 |
| --- | --- |
| 向量长期记忆 | 在已授权聊天中结合语义与关键词检索，找回当前上下文之外的历史对话。 |
| 五种历史查询工具 | 查询原文、搜索语义、展开上下文、统计消息条数，以及查找最早或最晚的匹配消息。 |
| 按需联网搜索 | 获取最新网页信息，支持模型服务托管搜索或自托管 SearXNG。 |
| 图片理解 | 让多模态模型理解聊天图片和链接卡片主图，图片读取或视觉调用失败时降级为文本。 |
| 工作流角色 | 为不同自动化设置角色昵称，标识 Bot 发言归属，帮助模型区分同一聊天中的不同角色。 |
| 双窗口上下文 | 用基础窗口与缓冲窗口保留近期原文，达到阈值后批量移出旧消息，减少上下文频繁变化。 |
| 可视化工作流 | 在画布中连接触发、条件、上下文、AI 和回复节点，编排群聊 Bot 与消息自动化。 |
| Agent 工具自主组合 | 由模型按问题需要多轮组合检索、统计和联网工具，共享可配置的次数、内容与时间预算。 |
| 多模型接入与回退 | 接入本地或云端 OpenAI 兼容服务，配置模型路由、重试与备用服务，减少单点故障影响。 |
| 执行全程可追溯 | 查看节点、模型用量、工具调用与回复状态，定位失败原因并处理待恢复任务。 |
